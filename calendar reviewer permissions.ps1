...
$ouusers = Get-ADUser -Filter * -SearchBase “OU=Training,OU=Users,OU=London,OU=Resources,DC=law,DC=firm,DC=priv”
...
$ouusers = Get-ADUser -Filter * -SearchBase “OU=Fee Earning Departments,OU=Users,OU=London,OU=Resources,DC=law,DC=firm,DC=priv”
Foreach ($Mailbox in $ouusers)
{
$path = $Mailbox.userprincipalname + “:\” + (Get-MailboxFolderStatistics $Mailbox.userprincipalname | Where-Object { $_.Foldertype -eq “Calendar” } | Select-Object -First 1).Name
Set-mailboxfolderpermission -identity ($path) -user Default -Accessrights reviewer
...
$ouusers = Get-ADUser -Filter * -SearchBase “OU=Support Departments,OU=Users,OU=London,OU=Resources,DC=law,DC=firm,DC=priv”
Foreach ($Mailbox in $ouusers)
{
$path = $Mailbox.userprincipalname + “:\” + (Get-MailboxFolderStatistics $Mailbox.userprincipalname | Where-Object { $_.Foldertype -eq “Calendar” } | Select-Object -First 1).Name
Set-mailboxfolderpermission -identity ($path) -user Default -Accessrights reviewer
...
$ouusers = Get-ADUser -Filter * -SearchBase “OU=Secondees,OU=Users,OU=London,OU=Resources,DC=law,DC=firm,DC=priv”
Foreach ($Mailbox in $ouusers)
{
$path = $Mailbox.userprincipalname + “:\” + (Get-MailboxFolderStatistics $Mailbox.userprincipalname | Where-Object { $_.Foldertype -eq “Calendar” } | Select-Object -First 1).Name
Set-mailboxfolderpermission -identity ($path) -user Default -Accessrights reviewer
